...
//active Win and Get Position
CGPoint GetPosition(NSString *title);
bool WinIsLaunch(NSString *title);
...
CGPoint GetPosition(NSString *title){
    ...
    for (NSMutableDictionary* entry in arr){
        ...
        _find_count2 +=1;
        if(_FIND_COUNT != _find_count2) continue;
        //属于自己app的窗口
        CGRect bounds;
        CGRectMakeWithDictionaryRepresentation((CFDictionaryRef)[entry objectForKey:@"kCGWindowBounds"], &bounds);
        if(DEBUG_F) NSLog(@"bounds: %@",NSStringFromRect(bounds));
