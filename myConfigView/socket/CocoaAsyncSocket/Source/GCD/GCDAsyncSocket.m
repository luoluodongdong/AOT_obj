...
- (BOOL)acceptOnUrl:(NSURL *)url error:(NSError **)errPtr;
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        self->acceptUNSource = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, socketUN, 0, socketQueue);
...
- (void)ssl_shouldTrustPeer:(BOOL)shouldTrust stateIndex:(int)aStateIndex
{
	...
	if (shouldTrust)
	{
        NSAssert1(lastSSLHandshakeError == errSSLPeerAuthCompleted, @"ssl_shouldTrustPeer called when last error is %d and not errSSLPeerAuthCompleted", (int)lastSSLHandshakeError);
		[self ssl_continueSSLHandshake];
	}
	else
	{
